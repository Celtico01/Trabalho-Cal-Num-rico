...
public class MetodoNewton extends MetodoEncontrarRaiz{
	private String[][] dados = new String[20][10];
	private int linha;
	private int coluna;
	...
	protected double derivada(double x) { // funcao derivada
	...
	public double mtdNewton() {
		...
		while(getErro() > getPrecisao() && getInteracoes() > getContador()) {
			...
			this.coluna++;
			guardaDados(derivada(getRaiz()), linha, coluna);
			this.coluna++;
